...
"""
注意xlrd目前不支持xlsx，可支持xls，好像对2003+以上的版本支持不咋滴，2003版可以
"""
...
class Writer:
    ...
    def write(self, r, c, value):
        ...
        if cell:
            ...
            if ncell:
                # 替换写入后的单元格格式与源文件srcfile中的对应单元格格式一致
...
if __name__ == '__main__':
    # reader = Reader()
    # reader.open_excel('../lib/cases/WEB.xls')
    ...
    # writer = Writer()
    # writer.copy_open('../lib/cases/HTTPLogin.xls', '../lib/results/result-HTTP接口用例.xls')
    ...
    reader.set_sheet(sheets[0])
    print(reader.readline())
    print(reader.readline())
    print(reader.readline())
    print(reader.readline())
    print(reader.readline())
    text = (reader.readline())[4]
    print(text)
    print(type(text))
